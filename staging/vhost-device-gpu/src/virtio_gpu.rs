...
mod tests {
    ...
    rusty_fork_test! {
        ...
        fn test_update_cursor_fails() {
            ...
            // fails to update cursor pos from frontend
        ...
        fn test_move_cursor_fails() {
            ...
            // No resources exists, but the test fails to set cursor pos from frontend
            let result = virtio_gpu.move_cursor(0, cursor_pos);
            assert_matches!(result, Err(ErrUnspec));

            // Resources exists, but the test fails to set cursor pos from frontend
